...
class Config:
    ...
    # SQLALCHEMY_DATABASE_URI = f"postgresql://{database_user}:{database_password}@{database_url}:{database_port}/{database_name}"
    ...
    REDIS_URL = (
        f"redis://:{redis_password}@{redis_url}:{redis_port}/0"
        if redis_password
        else f"redis://{redis_url}:{redis_port}/0"
    )
